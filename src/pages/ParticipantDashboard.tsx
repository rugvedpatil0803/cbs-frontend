...
const BOOKING_BASE = "http://localhost:8080/api/booking";
const FEEDBACK_BASE = "http://localhost:8080/api/feedback";
...
useEffect(() => {
  ...
  const fetchAll = async () => {
    try {
      ...
      const [up, on, comp, bookingsRes] = await Promise.all([
        fetch(`${API_BASE}/upcoming`, { headers }),
        fetch(`${API_BASE}/ongoing`, { headers }),
        fetch(`${API_BASE}/completed`, { headers }),
        fetch(`${BOOKING_BASE}/my-bookings`, { headers }),
      ]);
      ...
      const enrolledIds =
        bookingsData.data?.map((b: BookingItem) => b.sessionId) || [];
...
const fetchMyEnrollments = async () => {
    try {
        ...
        fetch(`${BOOKING_BASE}/my-bookings`, { headers }),
        fetch(`${FEEDBACK_BASE}/user`, { headers }),
        ]);

        const bookingData = await bookingRes.json();
        const feedbackData = await feedbackRes.json();

        setMyEnrollments(bookingData.data || []);
        setMyFeedbacks(feedbackData.data || []);
...
const handleEnroll = async (sessionId: number) => {
  ...
  try {
    ...
    const res = await fetch(
      `${BOOKING_BASE}/unenroll/${booking.sessionId}`,
      {
        method: "DELETE",
        headers,
      }
    ...
    Swal.fire({
      icon: "error",
      title: "Error",
      text: "Something went wrong while unenrolling",
    });
  }
};
